...
DB_PATH="$HOME/.clawopt/clawopt.sqlite"
WORKSPACE_BASE="$HOME/.openclaw"
HAS_SYSTEMCTL=0
if command -v systemctl &>/dev/null; then
    HAS_SYSTEMCTL=1
fi
SERVICES=$(ls $SERVICE_DIR/clawopt-*.service 2>/dev/null || true)
if [ -f "$SERVICE_DIR/clawopt.service" ]; then
    SERVICES="$SERVICES $SERVICE_DIR/clawopt.service"
fi
...
echo -e " - $HOME/.clawopt (本项目专用数据库及运行时数据)"
[ -d "$HOME/.clawopt_release" ] && echo -e " - ~/.clawopt_release (旧版数据)"
echo ""
...
rm -rf "$HOME/.clawopt"
rm -rf "$HOME/.clawopt_release"
[ -d "$HOME/.clawopt_dev" ] && rm -rf "$HOME/.clawopt_dev"
