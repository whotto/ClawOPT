...
export function EntryTable({ entries, activePath, writable, canPreview, downloadUrl, onOpen, onPreview, onRename, onCopy, onDelete }: Props) {
        ...
        return (
          <div key={entry.path} className={`flex items-center gap-2 px-3 py-2 ${activePath === entry.path ? 'bg-blue-50/60' : 'hover:bg-gray-50'}`}>
            <button type="button" onClick={() => onOpen(entry)} className="flex items-center gap-2 min-w-0 flex-1 text-left">
              ...
              <span className="truncate text-sm text-gray-800 font-mono">{entry.name}</span>
              {git && (
                <span title={t(`fileManager.git.${entry.git!.status}`)}>
            ...
            <span className="hidden md:block w-44 text-right text-xs text-gray-400 shrink-0">{formatTime(entry.mtimeMs, i18n.language)}</span>
