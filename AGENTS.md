...
- `frontend/`: Web UI，包含单聊、群聊、设置、模型管理、文件预览等功能。`frontend/src/` 下：
  ...
  - `features/`: 页面内功能块。`chat/`（`hooks/` 状态与副作用、`components/` 展示层、`lib/` 纯函数、`message/` 消息气泡与过程块）、`files/`（文件预览，按格式分查看器）、`workflow/`（工作流画布 `@xyflow/react`、运行回放、定时 / 钩子 / 导入导出弹窗；`lib/graph.ts` 的保存校验是服务端编译器的镜像，reason 码一致；`lib/nodeRuntime.ts` 外部运行时节点的模式与模型）、`approvals/`（运行审批：待办中心与聊天页输入区上方的审批卡共用的列表 hook 与卡片）。
