...
**分享**：同一个面板上点「分享（生成链接）」，包会上传成**你自己 GitHub 账号下的**一个私密 gist，
换回一条链接直接发给对方。本项目不存任何包——托管在你自己那边，删不删也由你。
需要主机上装了 `gh` 并已 `gh auth login`；没装就用「导出」下载文件自己发。

> 私密 gist 不等于私有：拿到链接的人都能看。带长期记忆的包别随手分享。
