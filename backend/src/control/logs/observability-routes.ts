...
/**
 * 用量 / 日志 / 网关状态卡。日志带运行现场（路径、会话 key），只给 admin；
 * 用量与状态卡登录即可看。网关重启沿用既有的 `POST /api/config/restart`（带重启状态机），这里不另开一条。
 */
export function registerObservabilityRoutes(app: RouteApp, ctx: ObservabilityRoutesDeps): void {
  ...
  app.get('/api/usage/summary', controlHandler(async (req, res) => {
