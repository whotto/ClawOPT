...
import type { ManagedMcpServer, RegisteredProxyTarget, RuntimeDescriptor } from '../_platform-types';
...
import { runtimeHomeDir } from './runtime-home';
    ...
    start(context: AdapterRunContext<CodingAgentRunRequest>): AdapterRunHandle {
      ...
      const run = async (): Promise<AdapterRunOutcome> => {
        ...
        try {
          ...
          homeDir = runtimeHomeDir(deps.dataDir, runtimeId, request.conversation);
          ...
          const mcp = await deps.mcp.resolveForRun({ runtime: runtimeId, userServers: request.userMcpServers ?? [] });
