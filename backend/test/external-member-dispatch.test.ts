...
import { harness, MemoryFs } from './runtime/adapters/_helpers/harness';
...
describe('会话生命周期', () => {
  ...
  it('库里的运行时没有对应适配器：消息里说清楚，不假装在跑', async () => {
    const engine = makeEngine();
    engine.useCodingAgentAdapters(() => undefined);
...
describe('外部成员经运行协调器', () => {
  ...
  it('会话键按 (群, 成员)，终态后写结束标记；result 的整轮用量按确定性 id 记一次；工具调用与结果成组落库', async () => {
    ...
    expect([...sharedFs.dirs].some((dir) => dir.includes('/runtime/claude-code/group/g1_'))).toBe(true);
  });
