 ...
 *
 * 58 → 59（P2 适配器）：`runtime/adapters/_shared/session-state.ts` 经运行时 home 网关写 `.clawopt-session.json`。
 * 路径 = 运行时 home（`<数据目录>/runtime/<运行时>/<哈希>`，不是数据给的）+ 固定文件名。
 *
 * 59 → 60（P2-platform，集成时合并两边各自的 58 → 59）：`runtime/platform-store.ts::readPrivateText` 是运行时平面
 * （代理恢复文件、本机密钥、更新策略、成员密钥、原生配置文件编辑器）唯一的读入口。路径要么是 ClawOPT 自己数据目录下的
 * 固定文件，要么来自运行时描述符里写死的原生配置路径表，不来自请求数据；读之前 stat 判普通文件（不跟命名管道较劲）。
 */
const CURRENT_SITE_BUDGET = 60;
