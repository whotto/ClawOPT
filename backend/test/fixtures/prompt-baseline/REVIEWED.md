...
| 2026-09-15（ACL 收口） | 群聊叫起按发起人授权：`resolveTargetAgentIds` / `dispatchExistingUserMessage` / `sendUserMessage` / `rerunUserMessage` 多一个 `WakePredicate` 参数（缺省全放行），只筛用户发起的第一跳叫起谁；新增只读的 `blockedMentions` 与 `canWakeMemberRef`。`sendToAgent` 与 `buildAgentPrompt` 一行未改，链式转交不受影响。 | 七份快照字节相同（本次运行「七种输入的字节快照与签入的基线逐字节一致」通过）。改动只决定调不调 `sendToAgent`，不改传给它的参数，也不经过 prompt 组装。红线 A 未被破坏。 | <1 分钟（同上） |
