...
describe('运行时 home', () => {
  it('群聊按 (群, 成员) 稳定且互不相同；单聊按会话键哈希', () => {
    const a = runtimeHomeDir('/d', 'codex', { kind: 'group', roomId: 'room/1', memberId: 'm 1' });
    expect(a).toBe(runtimeHomeDir('/d', 'codex', { kind: 'group', roomId: 'room/1', memberId: 'm 1' }));
    expect(a).not.toBe(runtimeHomeDir('/d', 'codex', { kind: 'group', roomId: 'room/1', memberId: 'm 2' }));
    expect(a.startsWith('/d/runtime/codex/group/room_1_')).toBe(true);
    expect(runtimeHomeDir('/d', 'pi', { kind: 'session', sessionKey: 's' })).toMatch(/^\/d\/runtime\/pi\/[0-9a-f]{32}$/);
    expect(() => runtimeHomeDir('/d', '../x', { kind: 'session', sessionKey: 's' })).toThrow();
  });
});
