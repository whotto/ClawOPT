...
import { runtimeHomePath, type RuntimeHomeOwner } from '../../../../src/runtime/manager/runtime-homes';
import type { RuntimeDescriptor, RuntimeManager } from '../../../../src/runtime/manager/types';
import type { ManagedMcpServer, McpInjector } from '../../../../src/runtime/mcp/types';
import type { ProviderProxy, ProxyTarget } from '../../../../src/runtime/proxy/types';
...
/** 平台的运行时目录根（`<数据目录>/runtime`）。 */
export const RUNTIME_ROOT = `${DATA_DIR}/runtime`;
/** 用例默认的归属：单聊会话 s1。 */
export const SESSION_OWNER: RuntimeHomeOwner = { kind: 'session', sessionId: 's1' };

/** 与平台 RuntimeHomes 同一个路径算法，只是不碰真实文件系统。 */
export function homeFor(runtime: string, owner: RuntimeHomeOwner = SESSION_OWNER): string {
  return runtimeHomePath(RUNTIME_ROOT, runtime, owner);
}
...
export function harness(options: {
  ...
  const deps: CodingAgentAdapterDeps = {
    ...
    homes: { ensureHome: (runtime, owner) => homeFor(runtime, owner) },
    logger: {
