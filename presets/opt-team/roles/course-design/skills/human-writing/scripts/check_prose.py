...
ROAD_SIGN_LITERAL_EXCLUSIONS = {
    "还有一层": ("楼", "皮", "膜", "漆", "纸"),
}

ROAD_SIGN_PATTERNS = tuple(
    re.compile(
        re.escape(phrase)
        + (
            "(?!{})".format("|".join(ROAD_SIGN_LITERAL_EXCLUSIONS[phrase]))
            if phrase in ROAD_SIGN_LITERAL_EXCLUSIONS
            else ""
        )
        + r"[^。！？!?\n]{0,24}"
    )
...
EXEMPT_COLON_PATTERNS = (
    # 剧本人物名后的冒号：短标签，且不含常见转述动词，避免把“他说：”一类叙述句误判成台词格式。
    re.compile(
        r"(?:^|\n)[^\n#：:，。！？!?说道讲问答喊叫念写想笑叹骂哭]{1,6}：",
        re.MULTILINE,
    ),
...
def exempt_colon_spans(text: str):
    """剧本人物名后的冒号、钟点时刻里的冒号不算违规。"""

    spans = []
    for pattern in EXEMPT_COLON_PATTERNS:
...
def main() -> int:
    ...
    colon_exempt_spans = exempt_colon_spans(prose)
    for symbol, label in FORBIDDEN_PUNCTUATION.items():
        ...
        if symbol in ("：", ":"):
            matches = [
                ...
                if not any(
                    start <= match.start() < end for start, end in colon_exempt_spans
                )
            ]
        if matches:
    ...
    if len(marker_matches) > marker_limit:
        ...
        warnings.append(
            f"软性判断词共 {len(marker_matches)} 处，当前提醒线为 {marker_limit} 处。"
            f"这类词只是提醒复核论据，不是硬性失败项。重点检查 {samples}。"
        )
    ...
    if metaphors:
        ...
        warnings.append(
            f"八百字内出现 {len(fields)} 套借喻迹象。{'、'.join(sorted(fields))}。"
            f"例词有 {samples}。所写对象真是这些实物或场景时，忽略这条提醒。"
        )
    ...
    print(
        ...
        f"需辨语境词 {len(context_jargon_matches)}，软性判断词 {len(marker_matches)}，"
