...
|------|--------|------|------|--------------------------|
| `culture-audit` | 检查 Agent 在压力下有没有偏离既定原则——说话纪律破功、边界被越过、开始迎合用户、 | v1.0 | 199 行 | `Read Grep Glob Write memory_search memory_get` |
...
| `skill-assessment` | 盘点 AI 团队的技能配置——评估每个技能的成熟度、找出能力覆盖缺口与重叠、 | v1.0 | 179 行 | `Read Grep Glob Write memory_search memory_get` |
| `team-health-diagnosis` | 给搭档的 AI Agent 团队做健康诊断——五维扫描（配置完整度/技能完备度/使用活跃 | v1.0 | 243 行 | `Read Grep Glob Write memory_search memory_get` |
