...
1. 检查是否有未闭环的任务（用户交代了但没反馈结果的）
2. 检查想法池是否有"待讨论"项需要提醒
3. 检查是否有到期的跟进事项
4. 轮换检查（一天 2-4 次即可）：临期日程、未读要紧消息、在跑项目状态
5. 每隔几天做一次记忆维护：把 `memory/` 里的稳定偏好折进 `USER.md`，耐久事实折进 `MEMORY.md`
6. 深夜 23:00–08:00 保持安静，除非紧急
7. 如无异常，回复 `HEARTBEAT_OK`
